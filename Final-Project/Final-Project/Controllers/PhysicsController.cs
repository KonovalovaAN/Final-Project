        ...
        public static bool IsCollide(Entity entity, Point dir)
        {
            ...
            for (int i = 0; i < MapController.mapObjects.Count; i++)
            {
                ...
                if (Math.Abs(delta.X) <= entity.size / 2 + currObject.size.Width / 2)
                {
                    ...
                    {
                        if (delta.X < 0 && dir.X == Game.GetSpeed() && entity.posY + entity.size / 2 > currObject.position.Y && entity.posY + entity.size / 2 < currObject.position.Y + currObject.size.Height)
                        {
                            return true;
                        }
                        if (delta.X > 0 && dir.X == -1 * Game.GetSpeed() && entity.posY + entity.size / 2 > currObject.position.Y && entity.posY + entity.size / 2 < currObject.position.Y + currObject.size.Height)
                        {
                            return true;
                        }
                        if (delta.Y < 0 && dir.Y == Game.GetSpeed() && entity.posX + entity.size / 2 > currObject.position.X && entity.posX + entity.size / 2 < currObject.position.X + currObject.size.Width)
                        {
                            return true;
                        }
                        if (delta.Y > 0 && dir.Y == -1 * Game.GetSpeed() && entity.posX + entity.size / 2 > currObject.position.X && entity.posX + entity.size / 2 < currObject.position.X + currObject.size.Width)
                        {
                            return true;
                        }
                    }
